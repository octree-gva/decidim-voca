...
export const updateActiveUploads = (modal) => {
  ...
  addFiles.forEach((file, ix) => {
    ...
    if (isTitled) {
      const titleValue =
        modal.modal.querySelectorAll('input[type="text"]')[ix].value;
      ...
      hidden += `<input type="hidden" name="${titleField}" value="${escapeQuotes(
        titleValue
      )}" />`;
    ...
    const template = `
      <div ${attachmentIdOrHiddenField} data-filename="${escapeQuotes(
      file.name
    )}" data-title="${escapeQuotes(title)}">
        ${
          (/image/.test(file.type) &&
            '<div><img src="data:," role="presentation" /></div>') ||
          ""
        }
        <span>${escapeHtml(title)} (${escapeHtml(
  ...
  attachmentButtons.forEach((attachmentButton) => {
    ...
    modal.dropZone.addEventListener(
      ...
      (event) =>
        event.preventDefault() ||
        resetDropzone(modal) ||
        modal.uploadFiles(event.dataTransfer.files)
    );
