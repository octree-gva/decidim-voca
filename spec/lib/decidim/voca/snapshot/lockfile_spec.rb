      ...
      describe Lockfile do
        ...
        describe ".generate" do
          ...
          before do
            ...
            allow(described_class).to receive(:read_npm_lock).and_return(npm_lock_json)
          end
        ...
        describe ".validate" do
          ...
          context "when lockfile doesn't match" do
            ...
            it "returns true (has errors)" do
              allow(described_class).to receive(:extract_decidim_modules).and_return({})
              allow(described_class).to receive(:read_npm_lock).and_return({ "packages" => {} }.to_json)
        ...
        describe ".errors" do
          context "when lockfile does not exist" do
            it "returns empty array" do
              expect(described_class.errors(File.join(temp_dir, "nonexistent.lock"))).to eq([])
            end
          end
          ...
          context "with dummy lockfile (symbol keys normalized)" do
            ...
            it "reports missing decidim modules when current has fewer" do
              ...
              allow(described_class).to receive(:read_npm_lock).and_return(npm_lock_with_lodash)

              errs = described_class.errors(lockfile_path)
            ...
            it "returns no errors when current has all target modules and npm packages" do
              ...
              expect(described_class.errors(lockfile_path)).to eq([])
            end
            ...
            it "reports npm package version mismatch" do
              ...
              allow(described_class).to receive(:read_npm_lock).and_return(
                {
                  "packages" => {
        ...
        describe ".validate!" do
          it "raises with joined errors when lockfile has missing modules" do
            ...
            expect { described_class.validate!(lockfile_path) }.to raise_error(/Missing module decidim-core/)
          end
          ...
          it "raises when module exists but at wrong version" do
            ...
            expect { described_class.validate!(lockfile_path) }.to raise_error(/Version mismatch for decidim-core: lockfile 0\.29\.10, current 0\.29\.2/)
          end
          ...
          it "raises when lockfile has missing npm package" do
            ...
            expect { described_class.validate!(lockfile_path) }.to raise_error(/Missing npm package lodash/)
          end
          ...
          it "raises when npm package exists but at wrong version" do
            ...
            expect { described_class.validate!(lockfile_path) }.to raise_error(/Version mismatch for npm package lodash: lockfile 4\.17\.21, current 4\.17\.20/)
          end
        end
        ...
        describe ".normalize_hash" do
          it "stringifies keys and recursively normalizes nested hashes" do
            ...
            expect(described_class.normalize_hash(input)).to eq(
              "foo" => "a", "bar" => { "baz" => "b" }
            )
          end
        ...
        describe ".extract_npm_packages" do
          it "returns empty hash for nil or empty content" do
            expect(described_class.extract_npm_packages(nil)).to eq({})
            expect(described_class.extract_npm_packages("")).to eq({})
          end
          ...
          it "extracts only root dependencies (names and versions) from package-lock v3" do
            ...
            result = described_class.extract_npm_packages(package_lock)
