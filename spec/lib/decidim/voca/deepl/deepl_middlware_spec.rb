  ...
  module Voca
    describe DeepL::Middleware do
      ...
      before do
        ...
        allow(Decidim::Voca::DeepL::Context).to receive(:current_locale=)
        allow(Decidim::Voca::DeepL::Context).to receive(:attributes).and_return({})
      end
      ...
      describe "#call" do
        ...
        context "when deepl is disabled" do
          ...
          it "skips deepl context setting" do
            expect(Decidim::Voca::DeepL::Context).not_to receive(:organization=)
            expect(Decidim::Voca::DeepL::Context).not_to receive(:participatory_space=)
            expect(Decidim::Voca::DeepL::Context).not_to receive(:current_component=)
            expect(Decidim::Voca::DeepL::Context).not_to receive(:current_locale=)
            subject.call(env)
          end
        end
        ...
        context "when deepl is enabled" do
          it "sets the deepl context and calls the app" do
            expect(Decidim::Voca::DeepL::Context).to receive(:organization=).with(organization.to_global_id.to_s)
            expect(Decidim::Voca::DeepL::Context).to receive(:participatory_space=).with(participatory_space.to_global_id.to_s)
            expect(Decidim::Voca::DeepL::Context).to receive(:current_component=).with(component.to_global_id.to_s)
            expect(Decidim::Voca::DeepL::Context).to receive(:current_locale=).with(I18n.locale.to_s)
        ...
        context "when an error occurs" do
          before do
            allow(Decidim::Voca::DeepL::Context).to receive(:organization=).and_raise(StandardError, "Test error")
